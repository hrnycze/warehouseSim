...
/*                                                                   */
/* Copyright (C) 2003-2019 by FANUC CORPORATION All rights reserved. */
/*                                                                   */
/*-------------------------------------------------------------------*/
...
/*
    Axis define
*/
/* machine information for Ethernet library */
...
#define MAX_AXIS        48
#define MAX_SPINDLE     16
#elif defined (FS30D) || defined (FS0IDD)
#define MAX_AXIS        32
...
#if !defined (MAX_AXIS)
#if defined (F22_TYPEB) || (!defined(CNC_PPC)&&!defined(WIN_XOS))
#define MAX_AXIS        96
#elif defined (F22_TYPEA)
#define MAX_AXIS        72
#elif defined (F22_TYPE5)
...
#if defined(F22_TYPEA) || defined(F22_TYPEB) || ((defined(PM_H)) && (!defined(DSA_SYS))) || !defined(CNC_PPC)
...
#if defined(F22_TYPEA) || (!defined(CNC_PPC)&&!defined(WIN_XOS))
#define MAX_CNCPATH     15
#else
#define MAX_CNCPATH     10
#endif
...
#define EW_TIMEOUT      22              /* time out error */
...
#define TIMEOUT_SECOND   53
#define SEARCHWORD_RESET 54
#define DSSRCH_RESET	 55
#define DSSRCH_PTR		 56
#define SLCTMAIN_MODE    57
#define DWNLOAD_MODE     58
#define CAS_EXEC_CONTROL 59
#define GALVANO_AXIS     60
#define WRACTPT_MDCHK    61
#define FSLOG_PUBLIC     62
#define CHECK_MACRO_NUM  63
#define PGLOCK_TYPE      64
#define HIS_NON_STOP_READ 65
#define HIS_NAME_READ    66
...
/*
    cnc_sysinfo_ex
*/
#define MPATH_CNC       0   /* Path attribute CNC     */
#define MPATH_LOADER    1   /* Path attribute Loader  */
#define MPATH_ROBOT     5   /* Path attribute Robot   */

/*
    cnc_rdltrendprcs, cnc_rdltrendamsr
*/
#define L_LC_NUM          12  /* Max LC unit num                   */
#define L_PSU_NUM         12  /* Max power suply unit num          */
#define L_FIBER_NUM       6   /* Max process fiber num             */
#define L_NRML_PWRTBL     9   /* Normal power table num            */
#define L_MIN_PWRTBL      9   /* Minute power table num            */
#define L_AMSR_NRML_STEP  8   /* Auto measurement normal step num  */
#define L_EX_DATA_NUM     10  /* External input data num           */
...
typedef struct odbaxdt {
    ...
    short   unit;       /* data unit */
    short   flag;       /* flags */
    char    robot_postype;  /* Robot position type */
    char    reserve;        /* reserve */
} ODBAXDT;
...
typedef struct odbophis3 {
    ...
    union {
        ...
        struct {
            ...
            char    pw_flag;    /* power on flag */
            short   pth_no;     /* path index */
            char    skey_name[13];/* pushed soft key name */
            char    dummy[3];   /* dummy                */
        } rec_skey;
    } u;
} ODBOPHIS3;
...
volatile    char    alm_msg[64];/* alarm message */
        ...
        struct {
            ...
            short   minute;     /* minute */
            short   second;     /* second */
            char    scrn_old_name[17];  /* old screen name */
            char    scrn_new_name[17];  /* new screen name */
        } rec_scrn2;
    } u;
} ODBOPHIS4;
...
typedef struct odbgrppos {
	...
	char	move_type ;		/* axis motion type */
...
typedef struct _out_ccies_devtype {
    unsigned short          Kind;
    unsigned char           pad[2];
    unsigned long           FunctionCcies;
} CCIES_DEVTYPE;
...
/*-----------------------------------*/
/* NET : CC-Link IE Field function   */
/*-----------------------------------*/

/* ccies_rdsetparam */
typedef struct _ccies_pmc_reg {
    unsigned short  Path;
    short           Kind;
    unsigned long   Address;
} CCIES_PMC_REG;

typedef struct _out_ccies_setprm {
    unsigned char   State;
    unsigned char   NetworkNo;
    unsigned short  StationNo;
    CCIES_PMC_REG   Status;
    unsigned char   DataOnAbnormal;
    unsigned char   Option1;
    unsigned char   pad1[2];
    CCIES_PMC_REG   RY;
    unsigned long   RYSize;
    CCIES_PMC_REG   RX;
    unsigned long   RXSize;
    CCIES_PMC_REG   RWw;
    unsigned long   RWwSize;
    CCIES_PMC_REG   RWr;
    unsigned long   RWrSize;
} OUT_CCIES_SETPRM;

/* ccies_wrsetparam */
typedef struct _in_ccies_setprm_flg {
    char            State;
    char            NetworkNo;
    char            StationNo;
    char            Status;
...
typedef struct _in_ccies_setprm_prm {
    unsigned char   State;
    unsigned char   NetworkNo;
    unsigned short  StationNo;
    CCIES_PMC_REG   Status;
...
typedef struct _in_ccies_setprm {
    IN_CCIES_SETPRM_FLG flg;
    IN_CCIES_SETPRM_PRM prm;
} IN_CCIES_SETPRM;

/* ccies_mntinfo */
typedef struct _out_ccies_mntinfo {
    char            Status;
    unsigned char   pad1[1];
    unsigned short  StatusDetail;
    char            TaskState[8];
    char            OperateState;
    unsigned char   NetworkNo;
    unsigned short  StationNo;
    unsigned long   CyclicRYSize;
    unsigned long   CyclicRXSize;
    unsigned long   CyclicRWwSize;
    unsigned long   CyclicRWrSize;
    char            BaudratePort1;
    char            BaudratePort2;
    unsigned char   pad2[2];
    char            MacAdress[12];
    unsigned short  VendorCode;
    unsigned short  DeviceType;
} OUT_CCIES_MNTINF;
...
/* cnc_rdltrendrec:read record information for LASER TREND FUNCTION */
typedef struct odbtrrec {
    unsigned short    from_map_ver;          /* FROM mapping version */
    unsigned short    prcs_strg_num;         /* Number of processing dataset */
    unsigned short    amsr_strg_num;         /* Number of auto measurement dataset */
    short             reserve[11];           /* reserve */
} ODBTRREC ;

/* cnc_rdltrendprcs:read processing datasets for LASER TREND FUNCTION */
/* cnc_rdltrendamsr:read auto measurement datasets for LASER TREND FUNCTION */
typedef struct trendinf {
    long             dsp_soft_ver;         /* DSP software version                */
    unsigned long    save_num;             /* Number of saves from initialization */
	short            osc_type;             /* Oscillator model code               */
    short            lc_rated_pwr;         /* LC rated output                     */
	short            psu_slct;             /* Psu selection bit                   */
    short            pcb_type;             /* Oscillator interface PCB type       */
    short            pcb_ver;              /* Oscillator interface PCB version    */
    char             map_ver;              /* Trend data FROM map version         */
    char             dataset_ver;          /* Trend dataset version               */
    char             year;                 /* Data acquisition year               */
    char             month;                /* Data acquisition month              */
    char             day;                  /* Data acquisition date               */
    char             hour;                 /* Data acquisition hour               */
    char             min;                  /* Data acquisition minute             */
    char             sec;                  /* Data acquisition second             */
    char             mpath;                /* CNC path                            */
    char             hrv;                  /* HRV                                 */
    char             itpcycle;             /* ITP cycle                           */
    char             cnc_soft_series[5];   /* CNC system software series          */
    char             cnc_soft_ver[5];      /* CNC system software version         */
    char             reserve[5];           /* reserve                             */
} TRENDINF ;

typedef struct prcsref {
    short   clad_ref_peak;        /* Clad reflected light peak value                                    */
    short   clad_ref_scmd;        /* S command when detecting the clad reflected light peak value       */
    short   clad_ref_lcmd;        /* L command when detecting the clad reflected light peak value       */
    short   clad_ref_pcmd;        /* P command when detecting the clad reflected light peak value       */
    short   clad_ref_qcmd;        /* Q command when detecting the clad reflected light peak value       */
    short   clad_ref_gcode;       /* G code when detecting the clad reflected light peak value          */
    short   core_ref_peak;        /* Core reflected light peak value                                    */
    short   core_ref_scmd;        /* S command when detecting the core reflected light peak value       */
    short   core_ref_lcmd;        /* L command when detecting the core reflected light peak value       */
    short   core_ref_pcmd;        /* P command when detecting the core reflected light peak value       */
    short   core_ref_qcmd;        /* Q command when detecting the core reflected light peak value       */
    short   core_ref_gcode;       /* G code when detecting the core reflected light peak value          */
    short   clad_over_thrs;       /* The number of times the clad reflected light exceeds the threshold */
    short   core_over_thrs;       /* The number of times the core reflected light exceeds the threshold */
    short   regulation_num;       /* Number of times laser output regulation has occurred               */
    short   pctr_out_num;         /* Number of times Fn488.7 was output                                 */
    char    reserve[6];           /* reserve                                                            */
} PRCSREF ;

typedef struct prcspwr {
    short           pwr_cmd_max;                      /* Maximum power command value                             */
    short           crnt_moni_max;                    /* CH1 current monitor at maximum power command            */
    short           bc_pwr_max;                       /* BC unit output monitor at maximum power command         */
    short           lc_pwr_sum_max;                   /* LC unit output monitor total at maximum power command   */
    short           lc_pwr_max[L_LC_NUM];             /* LC unit output monitor 1 to 12 at maximum power command */
    short           ffbd_max[L_FIBER_NUM];            /* Process fiber SBL1 to 6 at maximum power command        */
    short           swmirr_sbl_max[L_FIBER_NUM];      /* Switching mirror SBL1 to 6 at maximum power command     */
    short           foldmirr_sbl_max;                 /* Folding mirror SBL at maximum power command             */
    short           cllmtr_sbl_max;                   /* Collimator SBL at maximum power command                 */
    unsigned short  fs_signals_max;                   /* Fiber selector related signals at maximum power command */
    char            reserve[6];                       /* reserve                                                 */
} PRCSPWR ;

typedef struct prcsscnr {
    short           aprtr_tmp_peak;                   /* Aperture temperature peak value                         */
    short           fld_mrr_tmp_peak;                 /* Folding mirror temperature peak value                   */
    short           out_pnl_tmp_peak;                 /* Output panel temperature peak value                     */
    short           in_wndw_sctl_peak;                /* Input protective window Scattered light peak value      */
    short           out_wndw_sctl_peak;               /* Output protective window Scattered light peak value     */
    char            reserve[4];                       /* reserve                                                 */
} PRCSSCNR ;

typedef struct prcsex {
    short   ex_data[L_EX_DATA_NUM];      /* External input data 1 to 10                  */
} PRCSEX ;

typedef struct prcsothr {
    unsigned long  total_ope_time;                   /* Total uptime                                         */
    unsigned long  bc_on_time;                       /* BC ON time                                           */
    unsigned long  guide_on_time;                    /* Guide laser ON time                                  */
    unsigned long  fbcpr_on_time;                    /* Fiber coupler beam ON time                           */
    unsigned long  psu_on_time[L_PSU_NUM];           /* PSU 1 to 12 ON time                                  */
    unsigned long  lc_on_time[L_LC_NUM];             /* LC 1 to 12 ON time                                   */
    short          wtr_crnt_mt;                      /* Total flow rate                                      */
    short          bc_flw_mntr;                      /* BC flow rate                                         */
    short          fdfb_wtflr_moni;                  /* Feed fiber flow rate                                 */
    short          fibre_wt_flr;                     /* Process fiber flow rate (fiber selector flow rate)   */
    short          bc_tmp_mntr1;                     /* BC temperature 1                                     */
    short          bc_tmp_mntr2;                     /* BC temperature 2                                     */
    short          bc_tmp_mntr3;                     /* BC temperature 3                                     */
    short          bc_tmp_mntr4;                     /* BC temperature 4                                     */
    short          case_tmprtr;                      /* Housing temperature                                  */
    short          wtr_temp_mt;                      /* Chiller water temperature                            */
    short          lc_rf_phtmp[L_LC_NUM];            /* LC reflected light absorption temperature 1 to 12    */
    short          lc_ht_sktmp[L_LC_NUM];            /* LC heat sink temperature 1 to 12                     */
    short          lc_cvtmp1[L_LC_NUM];              /* LC cavity temperature 1 1 to 12                      */
    short          lc_cvtmp2[L_LC_NUM];              /* LC cavity temperature 2 1 to 12                      */
    short          lc_cvtmp3[L_LC_NUM];              /* LC cavity temperature 3 1 to 12                      */
    short          psu_ht_sktp[L_PSU_NUM];           /* PSU heat sink temperature 1 to 12                    */
    short          case_humdty;                      /* Housing humidity                                     */
    short          nrml_pwrtbl[L_NRML_PWRTBL];       /* Normal power table # 0 to 8                          */
    short          min_pwrtbl[L_MIN_PWRTBL];         /* Minute power table # 0 to 8                          */
    short          pwrcmd_offset;                    /* Output command offset                                */
    short          pwrtbl_intrvl;                    /* Power table interval                                 */
    short          bcpwr_calib;                      /* BC unit output monitor input correction coefficient  */
    short          reserve[4];                       /* reserve                                              */
} PRCSOTHR ;

typedef struct odbtrprcs {
    TRENDINF    prcsinf;          /* Information when saving the processing datasets  */
    PRCSREF     prcsref;          /* Reflected light related data                     */
    PRCSPWR     prcspwr;          /* Data at maximum power command                    */
	PRCSSCNR    prcsscnr;         /* FANUC LASER SCANNER data                         */
    PRCSEX      prcsex;           /* External input data                              */
    PRCSOTHR    prcsother;        /* Other oscillator state data                      */
} ODBTRPRCS ;


typedef struct amsrdt {
    short   nrml_step_cmd[L_AMSR_NRML_STEP];                        /* Power command in normal step (normal power table #0 to #7) */
    short   nrml_step_crnt_cmd[L_AMSR_NRML_STEP];                   /* Current command in normal step                             */
    short   nrml_step_bcpwr[L_AMSR_NRML_STEP];                      /* BC unit output monitor in normal step                      */
    short   nrml_step_lcsum[L_AMSR_NRML_STEP];                      /* LC unit output monitor total in normal step                */
    short   nrml_step_bcref[L_AMSR_NRML_STEP];                      /* Core reflected light in normal step                        */
	short   nrml_step_cladref[L_AMSR_NRML_STEP];                    /* Clad reflected light in normal step                        */
    short   nrml_step_ffbd[L_FIBER_NUM][L_AMSR_NRML_STEP];          /* Process fiber SBL1 to 6 in normal step                     */
    short   nrml_step_swmirr_sbl[L_FIBER_NUM][L_AMSR_NRML_STEP];    /* Switching mirror SBL1 to 6 in normal step                  */
    short   nrml_step_foldmirr_sbl[L_AMSR_NRML_STEP];               /* Folding mirror SBL in normal step                          */
    short   nrml_step_cllmtr_sbl[L_AMSR_NRML_STEP];                 /* Collimator SBL in normal step                              */
    short   nrml_step_lcpwr[L_LC_NUM][L_AMSR_NRML_STEP];            /* LC unit output monitor 1 to 12 in normal step              */
    short   nrml_step_exdata[L_EX_DATA_NUM][L_AMSR_NRML_STEP];      /* External input data 1 to 10 in normal step                 */
    short   rated_step_cmd;                                         /* Power command in rated output step                         */
    short   rated_step_crnt_cmd;                                    /* Current command in rated output step                       */
    short   rated_step_bcpwr;                                       /* BC unit output monitor in rated output step                */
    short   rated_step_lcsum;                                       /* LC unit output monitor total in rated output step          */
    short   rated_step_bcref;                                       /* Core reflected light in rated output step                  */
	short   rated_step_cladref;                                     /* Core reflected light in rated output step                  */
    short   rated_step_ffbd[L_FIBER_NUM];                           /* Process fiber SBL1 to 6 in rated output step               */
    short   rated_step_swmirr_sbl[L_FIBER_NUM];                     /* Switching mirror SBL1 to 6 in rated output step            */
    short   rated_step_foldmirr_sbl;                                /* Folding mirror SBL in rated output step                    */
    short   rated_step_cllmtr_sbl;                                  /* Collimator SBL in rated output step                        */
    short   rated_step_lcpwr[L_LC_NUM];                             /* LC unit output monitor 1 to 12 in rated output step        */
    short   rated_step_exdata[L_EX_DATA_NUM];                       /* External input data 1 to 10 in rated output step           */
    short   reserve[18];                                            /* reserve                                                    */
} AMSRDT ;

typedef struct preamsr {
    short           wtr_crnt_mt;                /* Total flow rate                                      */
    short           bc_flw_mntr;                /* BC flow rate                                         */
    short           fdfb_wtflr_moni;            /* Feed fiber flow rate                                 */
    short           fibre_wt_flr;               /* Process fiber flow rate (fiber selector flow rate)   */
    ...
    short           psu_ht_sktp[L_PSU_NUM];     /* PSU heat sink temperature 1 to 12                    */
    short           case_humdty;                /* Housing humidity                                     */
    unsigned short  fs_signals;                 /* Fiber selector related signals                       */
    short           reserve[10];                /* reserve                                              */
} PREAMSR ;

typedef struct odbtramsr {
    TRENDINF    amsrinf;          /* Information when saving the auto measurement datasets   */
    AMSRDT      amsrdt;           /* Data got by auto measurement                            */
    PREAMSR     preamsr;          /* Data got immmediately before executing auto measurement */
} ODBTRAMSR ;
...
typedef struct odbaxis_exdgt {
    short       counter;        /* counter */
    short       type;           /* axis number */
    short       dummy[2];       /* dummy */
    ODBEXPOS    data[MAX_AXIS]; /* data value */
} ODBAXIS_EXDGT ;

typedef struct iodbfssptninfo {
    unsigned short  ptn_type;           /* type          */
    unsigned short  ptn_valid_num;      /* enable        */
    unsigned short  ptn_max_num;        /* max           */
             short  ptn_default;        /* default       */
             char   prm_basic;          /* basic         */
             char   dummy[7];           /* dummy         */
} IODBFSS_PTNINFO;

typedef struct iodbfssptnname {
    short           ptn_num;            /* ptn number      */
    char            ptn_valid;          /* ptn valid       */
    char            ptn_name_num;       /* ptn name number */
    char            dummy[4];           /* dummy           */
    char            ptn_name_str[24];   /* ptn name        */
} IODBFSS_PTNNAME;

typedef struct odbfssslctptnname {
    short           sl_ptn_num ;        /* select ptn number      */
    char            sl_ptn_name_num;    /* select ptn name number */
    char            dummy[5];           /* dummy                  */
    char            sl_ptn_name_str[24];/* select ptn name        */
} ODBFSS_SLCTPTNNAME;

typedef struct iodbfssptntunemoni {
    short           ptn_num;            /* ptn number               */
    short           ptn_tunemoni_val;   /* ptn tuning moniter value */
    char            ptn_valid;          /* ptn valid                */
    char            dummy[3];           /* dummy                    */
} IODBFSS_PTNTUNEMONI;
typedef struct iodbfssitem {
    unsigned long   attr2[MAX_AXIS] ;    /* attribute2               */
    unsigned long   attr ;               /* attribute                */
    unsigned char   type ;               /* type                     */
    char            dummy[7] ;           /* dummy                    */
    char            valid[MAX_AXIS] ;    /* valid                    */
    union {
        char        bdata ;
        char        cdata ;
        short       idata ;
...
typedef struct soctrgr {
    long    seq_no;      /* sequence number */
    long    dummy[3];    /* reserve */
} IDBSOCTRGR ;
/*---------------------*/
/* CNC lock function   */
/*---------------------*/
...
/*-------------------------*/
/* Fast Cycle-time setting */
/*-------------------------*/
typedef struct iodbfctsitemlist {
	short			type;
	short			axis;
	short			item_num;
	short			dummy;
	unsigned long	item_id[64];
} IODBFCTSITEMLIST ;

typedef struct iodbfctsitem {
	short			type;
	short			axis;
...
typedef struct iodbrdfctsmsgid {
	short			type;
	short			axis;
	unsigned long	item_id;
	long			msg_id[3];
...
/* read machine axis position4 */
FWLIBAPI short WINAPI cnc_machine4( unsigned short, short, short, short, ODBAXIS_EXDGT * ) ;
...
FWLIBAPI short WINAPI cnc_mcdp_create(unsigned short, long, long);
...
/* read program information */
FWLIBAPI short WINAPI cnc_exeprginf( unsigned short, ODBEXEPRGINF * );
...
/* Start position sampling */
FWLIBAPI short WINAPI cnc_start_grppos2( unsigned short );

/* Stop position sampling */
FWLIBAPI short WINAPI cnc_stop_grppos2( unsigned short );

/* Read position data */
FWLIBAPI short WINAPI cnc_rd_grppos2( unsigned short, short *, ODBGRPPOS *, short );
...
FWLIBAPI short WINAPI cnc_sdstartocp( unsigned short, long );

FWLIBAPI short WINAPI cnc_sdendocp( unsigned short );

FWLIBAPI short WINAPI cnc_sdclrocp( unsigned short );

FWLIBAPI short WINAPI cnc_sdtstartocp( unsigned short, long );

FWLIBAPI short WINAPI cnc_sdtendocp( unsigned short );

FWLIBAPI short WINAPI cnc_sdtclrocp( unsigned short );
...
/* make Safety Mainte information file for PROFINET IO Controller Safety function  */
FWLIBAPI short WINAPI pnc_outsafemntinfo( unsigned short, char * );
...
/* NET : CC-Link IE Field function */
/*---------------------------------*/
FWLIBAPI short WINAPI ccies_rdsetparam(unsigned short, OUT_CCIES_SETPRM *);

FWLIBAPI short WINAPI ccies_wrsetparam(unsigned short, IN_CCIES_SETPRM_FLG *, IN_CCIES_SETPRM_PRM *);

FWLIBAPI short WINAPI ccies_mntinfo(unsigned short, OUT_CCIES_MNTINF *);
...
/* allocate library handle s */
FWLIBAPI short WINAPI cnc_allclibhndls( const char *, unsigned short, char *, long, unsigned short * );
...
/* Read IndexAxis offset data list ( idxax pos-offset screen )*/
FWLIBAPI short WINAPI cnc_rdindexdata_ofs( unsigned short, short, short, short *, long * );
/* Write IndexAxis offset data list ( idxax pos-offset screen )*/
FWLIBAPI short WINAPI cnc_wrindexdata_ofs( unsigned short, short, short, long );
...
/* read record information for LASER TREND FUNCTION */
FWLIBAPI short WINAPI cnc_rdltrendrec( unsigned short, ODBTRREC * ) ;
/* read processing datasets for LASER TREND FUNCTION */
FWLIBAPI short WINAPI cnc_rdltrendprcs( unsigned short, unsigned short, unsigned short *, ODBTRPRCS * ) ;
/* read auto measurement datasets for LASER TREND FUNCTION */
FWLIBAPI short WINAPI cnc_rdltrendamsr( unsigned short, unsigned short, unsigned short *, ODBTRAMSR * ) ;
/*----------------------------------*/
...
FWLIBAPI short WINAPI cnc_rdalmblkinfo(unsigned short, char *, long *, short *, short *);
...
/*------------------------*/
/* Fine Surface Setting   */
/*------------------------*/
FWLIBAPI short WINAPI cnc_fss_rdptninfo(unsigned short, IODBFSS_PTNINFO* );
FWLIBAPI short WINAPI cnc_fss_wrptninfo(unsigned short, IODBFSS_PTNINFO* );
FWLIBAPI short WINAPI cnc_fss_rdptnname(unsigned short, unsigned short*, IODBFSS_PTNNAME* );
FWLIBAPI short WINAPI cnc_fss_wrptnname(unsigned short, unsigned short*, IODBFSS_PTNNAME* );
FWLIBAPI short WINAPI cnc_fss_rdptnadjst(unsigned short, short, short*, short*, short* );
FWLIBAPI short WINAPI cnc_fss_wrptnadjst(unsigned short, short, short );
FWLIBAPI short WINAPI cnc_fss_rdslctptnname(unsigned short , ODBFSS_SLCTPTNNAME* );
FWLIBAPI short WINAPI cnc_fss_wrptnslct(unsigned short, short );
FWLIBAPI short WINAPI cnc_fss_rdtunemoni(unsigned short, unsigned short*, IODBFSS_PTNTUNEMONI* );
FWLIBAPI short WINAPI cnc_fss_rditem(unsigned short, unsigned short, unsigned short*, short, short, IODBFSS_ITEM* );
FWLIBAPI short WINAPI cnc_fss_writem(unsigned short, unsigned short, unsigned short*, short, short, IODBFSS_ITEM* );
...
FWLIBAPI short WINAPI cnc_rdsoc_wave_start2(unsigned short, short, IDBSOCTRGR* );
...
FWLIBAPI short WINAPI cnc_rdsoc_tlattr2( unsigned short, short, short*, short, short*, short*, short*, ODBSOCTLATTR* );
FWLIBAPI short WINAPI cnc_rdsoc_tldat2 ( unsigned short, short, short*, short, short*, IODBSOCTLDAT*);
FWLIBAPI short WINAPI cnc_wrsoc_tldat2 ( unsigned short, short, short*, short, short*, char*, IODBSOCTLDAT*);
...
/* Fast Cycle-time setting */
/*-------------------------*/
FWLIBAPI short WINAPI cnc_rdfctsstat(unsigned short, IODBFCTSITEMLIST *);
FWLIBAPI short WINAPI cnc_rdfctsitem(unsigned short, short *, IODBFCTSITEM []);
FWLIBAPI short WINAPI cnc_wrfctsitem(unsigned short, short *, IODBFCTSITEM []);
FWLIBAPI short WINAPI cnc_wrfctfssadjust(unsigned short, unsigned long);
FWLIBAPI short WINAPI cnc_rdfctsmsgid(unsigned short, short *, IODBRDFCTSMSGID []);
FWLIBAPI short WINAPI cnc_rdfctfssinfo(unsigned short, IODBFCTFSSINFO *);
FWLIBAPI short WINAPI cnc_wrfctfssinfo(unsigned short, IODBFCTFSSINFO *);
...
/* reset counter */
typedef struct tag_ODBRSTCNTR{
    unsigned short  push;
    unsigned short  release;
} ODBRSTCNTR;

FWLIBAPI short WINAPI cnc_resetcntr(unsigned short, ODBRSTCNTR*);
...
/*---------------------------------*/
/* Auxiliary Function time measure */
/*---------------------------------*/
#ifndef CNC_PPC
#define AUXMEAS_PERIOD_START_TO_FINISH	0	 /* From measurement start to measurement end */

typedef struct tag_IDBSTARTAUX {
    unsigned short type;
    unsigned short range;
    unsigned short range_medium_num;
    unsigned short period;
} IDBSTARTAUX;

#define AUXMEAS_TYPE_STS_STOP	0
#define AUXMEAS_TYPE_STS_WAIT	1
#define AUXMEAS_TYPE_STS_EXEC	2

typedef struct tag_ODBINFOAUX {
    unsigned short type;
    ODDATETIME request;
    ODDATETIME start;
    ODDATETIME finish;
...
#define AUXMEAS_ADDRESS_ALL_CODE	(-1)

#define AUXMEAS_ADDRESS_M_CODE		0
#define AUXMEAS_ADDRESS_S_CODE		1
#define AUXMEAS_ADDRESS_T_CODE		2
#define AUXMEAS_ADDRESS_B_CODE		3

typedef struct tagODBAUXCODE {
    short address;
    short reserve;
    long code;
} ODBAUXCODE;

typedef struct tag_ODBAUXDATA {
    unsigned short number;
    short reserve;
    ODBAUXCODE auxcode;
    long ave;
...
typedef struct tag_ODBAUXREGNUM {
    unsigned short maxnum;
    unsigned short curnum;
} ODBAUXREGNUM;

typedef struct tag_IODBAUX {
    unsigned short number;
    short address;
    long s_code;
    long e_code;
...
#define AUXMEAS_DEL_TYPE_MESUREMENT	0
#define AUXMEAS_DEL_TYPE_EXCLUSIVE	1
#define AUXMEAS_DEL_TYPE_BOTH		2

FWLIBAPI short WINAPI cnc_startaux(unsigned short FlibHndl, IDBSTARTAUX *startaux);
FWLIBAPI short WINAPI cnc_canaux(unsigned short FlibHndl);
FWLIBAPI short WINAPI cnc_getstsaux(unsigned short FlibHndl, unsigned short *stsaux);
FWLIBAPI short WINAPI cnc_getinfoaux(unsigned short FlibHndl, ODBINFOAUX *infoaux);
FWLIBAPI short WINAPI cnc_getauxdatar(unsigned short FlibHndl, short address, unsigned short s_number, unsigned short *data_num, ODBAUXDATA *auxdata);
FWLIBAPI short WINAPI cnc_getauxoverdatar(unsigned short FlibHndl, unsigned short *data_num, ODBAUXCODE *auxcode);
FWLIBAPI short WINAPI cnc_getauxregnum(unsigned short FlibHndl, ODBAUXREGNUM *auxregnum);
FWLIBAPI short WINAPI cnc_setallexauxr(unsigned short FlibHndl, IODBAUX *aux, unsigned short *data_num);
FWLIBAPI short WINAPI cnc_addexauxr(unsigned short FlibHndl, IODBAUX *aux, unsigned short *data_num);
FWLIBAPI short WINAPI cnc_updexauxr(unsigned short FlibHndl, IODBAUX *aux, unsigned short *data_num);
FWLIBAPI short WINAPI cnc_delexauxr(unsigned short FlibHndl, unsigned short *number, unsigned short *data_num);
FWLIBAPI short WINAPI cnc_getexauxr(unsigned short FlibHndl, short address, unsigned short s_number, unsigned short *data_num, IODBAUX *aux);
FWLIBAPI short WINAPI cnc_getexauxregnum(unsigned short FlibHndl, unsigned short *maxnum);
FWLIBAPI short WINAPI cnc_delallauxdata(unsigned short FlibHndl, unsigned short type);
#endif
...
/*-------------------------------------*/
/* read informations of CNC parameter */
FWLIBAPI short WINAPI cnc_rdglvnparainfo( unsigned short, short, short*, short*, short*, ODBPARAIF2 * ) ;
/* read parameter */
FWLIBAPI short WINAPI cnc_rdglvnparam( unsigned short, short, short, short, short, IODBPSD * ) ;
/* write parameter */
FWLIBAPI short WINAPI cnc_wrglvnparam( unsigned short, short, short, IODBPSD * ) ;
/* read informations of CNC diagnose data */
FWLIBAPI short WINAPI cnc_rdglvndiaginfo( unsigned short, short, short*, short*, short*, ODBPARAIF2 * ) ;
/* read diagnosis data */
FWLIBAPI short WINAPI cnc_glvndiagnoss( unsigned short, short, short, short, ODBDGN * ) ;

/* USB program load */
FWLIBAPI short WINAPI cnc_usb_load_start(unsigned short, char *);
FWLIBAPI short WINAPI cnc_usb_load_poll(unsigned short, short *, long *);
FWLIBAPI short WINAPI cnc_usb_load_end(unsigned short);

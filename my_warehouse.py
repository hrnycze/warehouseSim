...
class Warehouse():
     ...
     def move(self, move):
          ...
          if from_id == to_id:
               ...
               return
          if (from_id == GROUND_ID or from_id == OUTPUT_ID or from_id < INPUT_ID or from_id >= len(self.state)) or (to_id < OUTPUT_ID or to_id >= len(self.state)):
          ...
          if to_id == OUTPUT_ID:
               self.output = np.insert(self.output, 0, object_to_move) #insert on the top of stack
     ...
     def get_moves(self):
          ...
          if self.isOutputProccesed:
               for from_id, from_stack in enumerate(self.state):
                    ...
                    if self.output.size < len(self.required_order) and object_a == self.required_order[-self.output.size - 1]: #object_a is in order as next item
                         moves.append((from_id, OUTPUT_ID)) # move object from top of from_stack to output
                    ...
                    for to_id, to_stack in enumerate(self.state):
                         ...
                         if len(from_stack) > 1 and to_stack.size == 0 and ((from_id, 0) not in moves):
                              moves.append((from_id, GROUND_ID)) ## move object from top of from_stack to ground
                              continue

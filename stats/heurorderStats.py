...
if __name__ == "__main__":
    for type_desk in range(21, 31):
        for i in range(1,11):
            print(f"{type_desk}/10 {i}/10")

            sim2file = True
            if sim2file:
                f = open("analyzeData/typeDeskStats.txt", "a")
            ...
            rnd_state = get_random_state(N,S, max_stack_items=100, num_type=type_desk)
            ...
            print(f"{type_desk} ; noH{rnd_order}; heurOrder{heur_order}", end="", file=f)
            for sid in range(4):
